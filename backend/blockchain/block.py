...
class Block:
    ...
    def from_json(block_json):
        """
        De-serialize a block's json representation back into a block instance.
        """
    ...
    def is_valid_block(last_block, block):
        ...
        - Difficulty must only adjust by maximum 1
        ...
        if hex_to_binary(block.hash)[:block.difficulty] != '0' * block.difficulty:
            raise Exception('The PoW requirement is not satisfied.')
        ...
        if block.hash != reconstructed_hash:
            raise Exception('The block has to be correct.')
